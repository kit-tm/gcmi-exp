...
public class Reactor {
  ...
  public void loop() {
    while (!Thread.interrupted()) {
      ...
      //      Set<SelectionKey> keysWithLargeSendQueue = selected.stream().filter(sk -> sk.attachment() instanceof Handler)
      //              .filter(h -> ((Handler) h.attachment()).sendQueueIsAboveLimit()).collect(Collectors.toSet());
      //      if (keysWithLargeSendQueue.size() > 0) {
      //        log.debug("In Large-Send-Queue mode!");
      //        for (SelectionKey key : keysWithLargeSendQueue) {
      //          ((Handler) key.attachment()).handleEvent();
      //        }
      //        selected.removeAll(keysWithLargeSendQueue);
      //        continue;
      //      }
