...
public class InterceptableForwarder {
  ...
  private void switchDisconnected(
    ...
    connectionPairContext.controllerHandler.close();
  }
  ...
  public void acceptConnectionFromSwitch(Handler switchHandler) {
    synchronized (upstreamConnectionLock) {
      ...
      if (!handlerContexts.containsKey(switchHandler)) {
      ...
      try {
        reactor.establish(
            upstreamAddress, callbackFactoryForNewControllerConnections(switchHandler));
      } catch (IOException e) {
  ...
  private void controllerDisconnected(
      Throwable throwable, ConnectionPairContext connectionPairContext) {
    connectionPairContext.switchHandler.close();
  }

  public void connectTo(@Nullable InetSocketAddress upstreamListenAddress) {
    ...
    synchronized (upstreamConnectionLock) {
      ...
      handlerContexts.values().stream().map(c -> c.switchHandler).forEach(h -> h.close());
      handlerContexts.clear();
    }
  }
  ...
  static class ConnectionPairContext {
    ...
    Consumer<OFMessage> switchSender;
    ...
    ConnectionPairContext(Handler switchHandler) {
      ...
      this.switchSender = new OFMessageSerializingConsumer(bytes -> switchHandler.send(bytes, null));
    }

    void setControllerHandler(Handler controllerHandler) {
      this.controllerHandler = controllerHandler;
      this.controllerSender = new OFMessageSerializingConsumer(bytes -> controllerHandler.send(bytes, switchHandler));
      this.switchSender = new OFMessageSerializingConsumer(bytes -> switchHandler.send(bytes, controllerHandler));
    }
