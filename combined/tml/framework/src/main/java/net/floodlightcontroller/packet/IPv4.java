...
public class IPv4 extends BasePacket {
   ...
   * values at the time serialize is called: -checksum : 0 -headerLength : 0 -totalLength : 0
   */
  @Override
  public byte[] serialize() {
    ...
    this.totalLength =
        (short) (this.headerLength * 4 + ((payloadData == null) ? 0 : payloadData.length));
    ...
    bb.putShort(
        (short)
            (((this.flags & IPV4_FLAGS_MASK) << IPV4_FLAGS_SHIFT)
  ...
  public IPacket deserialize(byte[] data, int offset, int length) throws PacketParsingException {
    ...
    if (this.version != 4) {
      throw new PacketParsingException("Invalid version for IPv4 packet: " + this.version);
    }
    ...
    IPacket payload;
    isFragment =
        ((this.flags & IPV4_FLAGS_DONTFRAG) == 0)
            && ((this.flags & IPV4_FLAGS_MOREFRAG) != 0 || this.fragmentOffset != 0);
    ...
    else this.isTruncated = false;

    return this;
  }

  /**
   * Accepts an IPv4 address of the form xxx.xxx.xxx.xxx, ie 192.168.0.1 and returns the
  ...
  public static int toIPv4Address(String ipAddress) {
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      if (oct > 255 || oct < 0)
        throw new IllegalArgumentException(
            "Octet values in specified" + " IPv4 address must be 0 <= value <= 255");
      result |= oct << ((3 - i) * 8);
    }
    return result;
  }
  ...
  /**
   * Accepts a collection of IPv4 addresses as integers and returns a single String useful in
   * toString method's containing collections of IP addresses.
   *
  ...
  public boolean equals(Object obj) {
    ...
    if (!(obj instanceof IPv4)) return false;
    IPv4 other = (IPv4) obj;
    if (checksum != other.checksum) return false;
    if (!destinationAddress.equals(other.destinationAddress)) return false;
    if (diffServ != other.diffServ) return false;
    ...
    if (!protocol.equals(other.protocol)) return false;
    if (!sourceAddress.equals(other.sourceAddress)) return false;
