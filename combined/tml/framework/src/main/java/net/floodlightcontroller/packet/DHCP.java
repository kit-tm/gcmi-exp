...
public class DHCP extends BasePacket {
  ...
  public DHCPOption getOption(DHCPOptionCode optionCode) {
    for (DHCPOption opt : options) {
      if (opt.code == optionCode.value) return opt;
    }
    return null;
  }
  ...
  public byte[] serialize() {
    ...
    if (optionsLength < 60) optionsPadLength = 60 - optionsLength;
  ...
  public IPacket deserialize(byte[] data, int offset, int length) {
    ...
    this.clientHardwareAddress =
        MacAddress.of(
            tmpMac); /* the assumption here is that we only have MAC address HW addresses */
    for (int i = hardwareAddressLength; i < 16; ++i) bb.get();
