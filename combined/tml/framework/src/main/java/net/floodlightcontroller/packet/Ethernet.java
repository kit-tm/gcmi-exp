/**
 * Copyright 2011, Big Switch Networks, Inc. Originally created by David Erickson, Stanford
 * University
 *
...
public class Ethernet extends BasePacket {
  ...
  public static final short VLAN_UNTAGGED =
      VlanVid.ZERO
          .getVlan(); // untagged vlan must be 0x0000 for loxi. We can use the convenient ZERO field
  ...
  public byte[] serialize() {
    ...
    int length =
        14 + ((vlanID == VLAN_UNTAGGED) ? 0 : 4) + ((payloadData == null) ? 0 : payloadData.length);
    if (pad && length < 60) {
      length = 60;
    }
  ...
  public IPacket deserialize(byte[] data, int offset, int length) {
    ...
    if (this.destinationMACAddress == null) this.destinationMACAddress = MacAddress.of(new byte[6]);
    byte[] dstAddr = new byte[MacAddress.NONE.getLength()];
    bb.get(dstAddr);
    this.destinationMACAddress = MacAddress.of(dstAddr);

    if (this.sourceMACAddress == null) this.sourceMACAddress = MacAddress.of(new byte[6]);
    ...
    if (Ethernet.etherTypeClassMap.containsKey((short) this.etherType.getValue())) {
      Class<? extends IPacket> clazz =
          Ethernet.etherTypeClassMap.get((short) this.etherType.getValue());
      try {
        ...
        if (log.isTraceEnabled()) {
          log.trace(
              ...
              new Object[] {
                this.sourceMACAddress, this.destinationMACAddress, clazz.getClass().getName()
              });
        ...
        if (log.isTraceEnabled()) {
          log.trace("Fail to instantiate class {}, {}", clazz.getClass().getName(), e);
        }
        this.payload = new Data(data);
      } catch (IllegalAccessException e) {
        if (log.isTraceEnabled()) {
          log.trace("Fail to access class for instantiation {}, {}", clazz.getClass().getName(), e);
        }
        this.payload = new Data(data);
  ...
  /**
   * Accepts a MAC address of the form 00:aa:11:bb:22:cc, case does not matter, and returns a
   * corresponding byte[].
   *
  ...
  /**
   * Accepts a MAC address and returns the corresponding long, where the MAC bytes are set on the
   * lower order bytes of the long.
   *
   * @param macAddress
  ...
  public int hashCode() {
    ...
    result =
        prime * result + ((destinationMACAddress == null) ? 0 : destinationMACAddress.hashCode());
  ...
  public boolean equals(Object obj) {
    ...
    if (destinationMACAddress == null) {
      if (other.destinationMACAddress != null) return false;
    } else if (!destinationMACAddress.equals(other.destinationMACAddress)) return false;
    if (etherType == null) {
      if (other.etherType != null) return false;
    } else if (!etherType.equals(other.etherType)) return false;
    if (pad != other.pad) return false;
    if (priorityCode != other.priorityCode) return false;
  ...
  public String toString() {
    ...
    if (pkt instanceof ARP) sb.append("arp");
    else if (pkt instanceof LLDP) sb.append("lldp");
    else if (pkt instanceof ICMP) sb.append("icmp");
    else if (pkt instanceof IPv4) sb.append("ip");
    else if (pkt instanceof DHCP) sb.append("dhcp");
    else sb.append(this.getEtherType().toString());

    sb.append("\ndl_vlan: ");
    if (this.getVlanID() == Ethernet.VLAN_UNTAGGED) sb.append("untagged");
    else sb.append(this.getVlanID());
    sb.append("\ndl_vlan_pcp: ");
