...
class RoutingProxy {
  ...
  private void receivedFromDownstream(byte[] bytes, Context context) {
    // log.info("switch -> controller: {}", Arrays.toString(bytes));
    context.upstreamHandler.send(bytes);
  }
  ...
  private void receivedFromUpstream(byte[] bytes, Context context) {
    // log.info("controller -> switch: {}", Arrays.toString(bytes));
    context.downstreamHandler.send(bytes);
  }
