...
class InterceptableForwarderSpec extends Specification {
    ...
    def "message is forwarded from controller to switch"() {
        ...
        then:
        1 * switchHandler.send(_) >> { args -> received = OFFactories.genericReader.readFrom(Unpooled.wrappedBuffer(args[0])) }
        ...
        then:
        1 * controllerHandler.send(_) >> { args -> received = OFFactories.genericReader.readFrom(Unpooled.wrappedBuffer(args[0])) }
        received.xid == 123
    }
}
