/**
 * Copyright 2015, Big Switch Networks, Inc. Originally created by David Erickson, Stanford
 * University
 *
...
public class IPv6 extends BasePacket {
  ...
  public byte[] serialize() {
    ...
    if (this.payload != null) {
      ...
      if ((this.nextHeader == null || this.nextHeader.equals(IpProtocol.NONE))) {
        if (IPv6.nextHeaderClassMap.containsValue(this.payload.getClass())) {
          Set<Map.Entry<IpProtocol, Class<? extends IPacket>>> entries =
              IPv6.nextHeaderClassMap.entrySet();
          for (Map.Entry<IpProtocol, Class<? extends IPacket>> m : entries) {
            if (m.getValue().equals(this.payload.getClass())) {
              ...
              log.warn(
                  "Setting previously unset IPv6 'next header' to {} as detected by payload {}",
                  m.getKey(),
                  this.getPayload().getClass().toString());
              break;
            }
          }
          ...
          throw new IllegalArgumentException(
              "IpProtocol is unset in IPv6 packet "
                  + this.toString()
                  + ". Unable to determine payload type to set for payload "
                  + this.getPayload().getClass().toString());
        }
      }
    }
    ...
    bb.put((byte) (((this.version & 0xF) << 4) | ((this.trafficClass & 0xF0) >>> 4)));
    bb.put((byte) (((this.trafficClass & 0xF) << 4) | ((this.flowLabel & 0xF0000) >>> 16)));
  ...
  public IPacket deserialize(byte[] data, int offset, int length) throws PacketParsingException {
    ...
    if (this.version != 6) {
      throw new PacketParsingException("Invalid version for IPv6 packet: " + this.version);
    }
    this.trafficClass = (byte) (((firstByte & 0xF) << 4) | ((secondByte & 0xF0) >>> 4));
    this.flowLabel = ((secondByte & 0xF) << 16) | (bb.getShort() & 0xFFFF);
    ...
    this.payload =
        payload.deserialize(
            data, bb.position(), Math.min(this.payloadLength, bb.limit() - bb.position()));
    this.payload.setParent(this);
    // We're done!
    return this;
  }
  ...
  public boolean equals(Object obj) {
    ...
    if (destinationAddress == null) {
      if (other.destinationAddress != null) return false;
    } else if (!destinationAddress.equals(other.destinationAddress)) return false;
    if (flowLabel != other.flowLabel) return false;
    if (hopLimit != other.hopLimit) return false;
    if (nextHeader == null) {
      if (other.nextHeader != null) return false;
    } else if (!nextHeader.equals(other.nextHeader)) return false;
