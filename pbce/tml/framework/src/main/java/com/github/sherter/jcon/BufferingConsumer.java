 ...
 * Chunker}
 */
public class BufferingConsumer implements Consumer<byte[]> {
  ...
  public void accept(byte[] data) {
    if (buffer == null) {
      buffer = ByteBuffer.allocate(allocationSizeHeuristic(0, 0, data.length));
    }
    if (buffer.remaining() < data.length) {
      ByteBuffer newBuffer =
          ByteBuffer.allocate(
              allocationSizeHeuristic(buffer.capacity(), buffer.remaining(), data.length));
      buffer.flip();
